...
void partitionGraphWithMETIS(const Graph& graph, int nparts, vector<int>& partition) {
    ...
    vector<idx_t> adjncy;
    vector<idx_t> adjwgt;  // Edge weights

    // Initialize xadj array
    xadj[0] = 0;
    for (int u = 0; u < nvtxs; ++u) {
        for (const auto& edge : graph.adj[u]) {
            adjncy.push_back(edge.to);
            adjwgt.push_back(static_cast<idx_t>(edge.weight * 100)); // Convert to integer weight
        }
    ...
    // Resize partition vector
    partition.resize(nvtxs);
    
    // Call METIS_PartGraphKway with correct parameters
    int ret = METIS_PartGraphKway(
        ...
        NULL,          // Vertex weights (NULL = all vertices have same weight)
        NULL,          // Size of vertices for computing communication volume
        adjwgt.data(), // Edge weights
        &metis_nparts, // Number of partitions
        NULL,          // Target partition weights
        NULL,          // Allowed load imbalance
        options,       // Array of options
        &edgecut,      // Output: Resulting edge-cut
        partition.data() // Output: Partition vector
    );
...
int main(int argc, char** argv) {
    ...
    // Need to broadcast each field of the struct separately
    for (int i = 0; i < changeSize; i++) {
        if (rank == 0) {
            int u = changes[i].u;
            int v = changes[i].v;
            double weight = changes[i].weight;
            int isInsertion = changes[i].isInsertion ? 1 : 0;
            
            MPI_Bcast(&u, 1, MPI_INT, 0, MPI_COMM_WORLD);
            MPI_Bcast(&v, 1, MPI_INT, 0, MPI_COMM_WORLD);
            MPI_Bcast(&weight, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
            MPI_Bcast(&isInsertion, 1, MPI_INT, 0, MPI_COMM_WORLD);
        } else {
            int u, v, isInsertion;
            double weight;
            ...
            changes[i].isInsertion = (isInsertion == 1);
        }
    }
